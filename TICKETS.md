...
| A4 | `[ ]` | **Stop "Goes in the pack" re-ticking itself when Form changes.** Untick it on a tablet, switch to capsule, and it silently comes back on — adding something to the pack the person removed. Fable 5's Fix 3. | Claude Code |
| A5 | `[ ]` | **Stop "Goes in the pack" clearing on a schedule round-trip.** Fixed → As needed → Fixed loses the tick. Fable 5's Fix 4. | Claude Code |
| A6 | `[ ]` | **Sort chosen days into week order.** Tapping Fri then Mon stores and shows "Fri, Mon". Fable 5's Fix 5. | Claude Code |
